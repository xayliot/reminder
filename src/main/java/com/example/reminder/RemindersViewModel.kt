...
class RemindersViewModel: ViewModel() {
        ...
        if (cursor?.moveToFirst() == true) {
            do {
                ...
                if (Utils.isReminderInPast(date, time)) {
                    val newDate = Utils.getNextDayDate(date)
                    updateReminderDate(reminder, newDate, context)
                } else {
                    reminders.add(reminder)
                }
    ...
    fun updateReminderDate(reminder: Reminder, newDate: String, context: Context) {
        dbHelper.writableDatabase?.update(DatabaseHelper.TABLE_NAME, ContentValues().apply {
            put(DatabaseHelper.COLUMN_DATE, newDate)
            put(DatabaseHelper.COLUMN_TIME, reminder.time)
        }, "${DatabaseHelper.COLUMN_ID}=?", arrayOf(reminder.id.toString()))

        reminder.date = newDate

        scheduleNotification(context, newDate, reminder.time, reminder.text, reminder.id)
    }

    var reminderList: List<Reminder> by mutableStateOf(emptyList())


    @RequiresApi(Build.VERSION_CODES.O)
    fun resetCompletionStatusIfNeeded() {
        val currentDate = LocalDate.now().toString()
        reminders.forEachIndexed { index, reminder ->
            if (reminder.date < currentDate) {
                reminders[index] = reminder.copy(isCompleted = false)
            }
        }
    }
